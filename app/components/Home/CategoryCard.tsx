...
export default function CategoryCard({
    ...
    const handleImageError = (e: React.SyntheticEvent<HTMLImageElement>) => {
        ...
        if (parent) {
            parent.classList.add('bg-primary-dark');
        }
    };
    const getVariantStyles = () => {
        switch (variant) {
            case 'large':
                return 'md:col-span-2 md:row-span-2 rounded-[24px] shadow-xl h-full';
            case 'tall':
    ...
    if (variant === 'wide') {
        return (
            <div className={`${getVariantStyles()} group relative overflow-hidden hover-lift cursor-pointer`}>
                <img alt={title} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" src={image} onError={handleImageError} />
                ...
                <div className="relative h-full p-6 flex flex-col justify-between z-10">
                    ...
                    <div className="flex justify-between items-end">
                        ...
                        {icon && <span className="material-icons-outlined text-white/80 text-4xl">{icon}</span>}
                    </div>
                </div>
            </div>
        );
